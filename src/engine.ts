...
export function placeDecos(p: Project, rnd: () => number, bias: MoodBias, count: number): DecoLayer[] {
  const boxes = p.devices.filter(d => d.visible).map(deviceBox); const { w: cw, h: ch } = p.canvas;
  const pool = DECO_PRESETS.filter(d => bias.decoCats.includes(d.cat)); const out: DecoLayer[] = []; let attempts = 0;
  while (out.length < count && attempts < count * 30) { attempts++; const preset = pick(rnd, pool.length ? pool : DECO_PRESETS); const corner = Math.floor(rnd() * 4); const m = 0.2; let x: number, y: number; if (corner === 0) { x = rngRange(rnd, 0.02, m); y = rngRange(rnd, 0.05, 0.95); } else if (corner === 1) { x = rngRange(rnd, 1 - m, 0.98); y = rngRange(rnd, 0.05, 0.95); } else if (corner === 2) { x = rngRange(rnd, 0.05, 0.95); y = rngRange(rnd, 0.02, m * 0.8); } else { x = rngRange(rnd, 0.05, 0.95); y = rngRange(rnd, 1 - m * 0.8, 0.98); }
    const scale = preset.cat === 'ui' ? rngRange(rnd, 0.05, 0.09) : rngRange(rnd, 0.04, 0.13); const r = scale * Math.min(cw, ch); const box = { x: x * cw - r, y: y * ch - r, w: r * 2, h: r * 2 }; if (boxes.some(b => b.x < box.x + box.w && b.x + b.w > box.x && b.y < box.y + box.h && b.y + b.h > box.y)) continue;
...
export function scoreDesign(p: Project): DesignScore {
  const { w: cw, h: ch } = p.canvas; const boxes = p.devices.filter(d => d.visible).map(deviceBox);
  let bx = 0, by = 0, area = 0; for (const b of boxes) { const a = b.w * b.h; bx += (b.x + b.w / 2) * a; by += (b.y + b.h / 2) * a; area += a; }
  const cx = area ? bx / area : cw / 2, cy = area ? by / area : ch / 2; const off = Math.hypot(cx - cw / 2, cy - ch / 2) / Math.hypot(cw / 2, ch / 2); const balance = Math.round(clamp(1 - off * 1.4, 0, 1) * 100);
  let spacing = 100; for (let i = 0; i < boxes.length; i++) for (let j = i + 1; j < boxes.length; j++) { if (boxes[i].x < boxes[j].x + boxes[j].w && boxes[i].x + boxes[i].w > boxes[j].x && boxes[i].y < boxes[j].y + boxes[j].h && boxes[i].y + boxes[i].h > boxes[j].y) spacing -= 30; }
  spacing = Math.round(clamp(spacing, 0, 100));
  const tc = p.text.autoColor ? textOn(p.background.c1) : p.text.color; const contrast = Math.round(Math.abs((parseInt(tc.slice(1), 16) & 0xff) - (parseInt(p.background.c1.slice(1), 16) & 0xff)) / 255 * 60 + 40);
  const covered = boxes.reduce((s, b) => s + Math.min(b.w * b.h, cw * ch), 0) / (cw * ch); const vis = 1 - Math.abs(covered - 0.42) * 1.8; const visibility = Math.round(clamp(vis, 0, 1) * 100);
  const total = Math.round(balance * 0.3 + spacing * 0.3 + contrast * 0.15 + visibility * 0.25); return { total, balance, spacing, contrast, visibility };
}
...
export function generateDesign(base: Project, opts: GenOpts): Project {
  const rnd = mulberry32(opts.seed >>> 0); const bias = MOODS[opts.mood] || MOODS.auto;
  let p: Project = JSON.parse(JSON.stringify({ ...base, thumbnail: null })); const { w: cw, h: ch } = p.canvas;
  ...
  const doText = !opts.locks.text && opts.mode === 'all'; const doLogo = !opts.locks.logo && opts.mode === 'all';
  if (doBg) { const palIdx = pick(rnd, bias.palettes); const pal = PALETTES[palIdx]; let style = pick(rnd, bias.styles); if (!style) style = 'studio'; const background: Background = { ...paletteToBg(pal, Math.floor(rnd() * 1e9)), style, angle: pal.angle + Math.floor(rngRange(rnd, -18, 18)), meshPoints: 3 + Math.floor(rnd() * 4), light: { type: pal.light, intensity: rngRange(rnd, 0.4, 0.75) }, pattern: rnd() > 0.6 ? pal.pattern : 'none', patternOpacity: pal.po }; p = { ...p, background, accents: { a1: pal.a1, a2: pal.a2 } }; }
  if (doDevices) { const usable = p.assets.length; let count = opts.deviceCount ?? clamp(usable || 1, 1, 4); if (bias.preferMulti) count = clamp(Math.max(count, 2), 2, 4); const pool = COMPOSITIONS.filter(c => c.slots.length === count); const fallback = COMPOSITIONS.filter(c => Math.abs(c.slots.length - count) <= 1); const comp = pick(rnd, pool.length ? pool : (fallback.length ? fallback : COMPOSITIONS)); const assets = p.assets; p = { ...p, devices: comp.slots.map((s, i) => { const prev = p.devices[i]; const d = makeDevice(s.k, cw, ch, assets[i % Math.max(1, assets.length)]?.id ?? null, i); return { ...d, x: s.x * cw, y: s.y * ch, w: s.w * cw, tilt: (s.t ?? 0) + Math.floor(rngRange(rnd, -bias.maxTilt / 2, bias.maxTilt / 2)), shadow: pick(rnd, bias.shadows), assetId: assets.length ? assets[i % assets.length].id : (prev?.assetId ?? null), color: prev?.color ?? d.color, z: i }; }) }; }
  if (doDeco) { const [lo, hi] = bias.density; const density = rngRange(rnd, lo, hi); const count = Math.round(clamp(density * 9, 0, 10)); p = { ...p, decos: placeDecos(p, rnd, bias, count), decoration: { ...p.decoration, density, set: 'none' } }; }
  ...
  p = { ...p, mood: opts.mood, updatedAt: Date.now() }; return p;
}
...
export function responsiveShowcase(p: Project, variant: number): Project {
  const { w: cw, h: ch } = p.canvas; const desktop = p.assets.find(a => a.w / a.h > 1.2); const tablet = p.assets.find(a => a.w / a.h >= 0.7 && a.w / a.h <= 1.2 && a !== desktop); const mobile = p.assets.find(a => a.w / a.h < 0.7);
  ...
  const comps = COMPOSITIONS.filter(c => c.cat === 'trio'); const comp = comps[variant % comps.length] || comps[0]; const kinds: DeviceKind[] = ['laptop', 'tablet', 'phone'];
