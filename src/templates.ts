import type { Asset, Background, BgStyle, BgType, DecoCat, DecoLayer, DecoPrim, DecoSet, DecoShape, DeviceKind, DeviceLayer, FitMode, LightType, Material, Mood, PatternKind, PosPreset, Project, ScreenRect, ShadowPreset, TextBlock } from './types';
...
export function shade(hex: string, amt: number): string { const n = parseInt(hex.replace('#', ''), 16); const c = (v: number) => clamp(Math.round(v + amt), 0, 255); const r = c((n >> 16) & 255), g = c((n >> 8) & 255), b = c(n & 255); return `#${((1 << 24) + (r << 16) + (g << 8) + b).toString(16).slice(1)}`; }
...
export function deviceGeometry(kind: DeviceKind, w: number, h: number, radiusMul = 1): ScreenRect {
  switch (kind) {
    case 'laptop': { const baseH = h * 0.062; const lidH = h - baseH; const x = w * 0.062, y = lidH * 0.06; return { x, y, w: w - x * 2, h: lidH - y - lidH * 0.055, r: Math.min(w, h) * 0.014 * radiusMul }; }
    ...
    case 'monitor': { const standH = h * 0.15; const screenH = h - standH; const pad = screenH * 0.028; return { x: pad, y: pad, w: w - pad * 2, h: screenH - pad * 2, r: Math.min(w, h) * 0.01 * radiusMul }; }
  }
}

export function computeFit(s: ScreenRect, iw: number, ih: number, fit: FitMode, zoom: number, panX: number, panY: number) {
  let dw: number, dh: number;
  if (fit === 'stretch') { dw = s.w; dh = s.h; } else { const sc = fit === 'cover' ? Math.max(s.w / iw, s.h / ih) : Math.min(s.w / iw, s.h / ih); dw = iw * sc; dh = ih * sc; }
  dw *= zoom; dh *= zoom;
  const cx = s.x + s.w / 2 + (panX * s.w) / 2; const cy = s.y + s.h / 2 + (panY * s.h) / 2;
...
export const ASPECTS = [{ label: '16:9', w: 1600, h: 900 }, { label: '4:3', w: 1440, h: 1080 }, { label: '1:1', w: 1200, h: 1200 }, { label: '4:5', w: 1080, h: 1350 }, { label: '9:16', w: 1080, h: 1920 }, { label: '3:2', w: 1500, h: 1000 }];
export const PROJECT_TYPES = ['Website', 'Web App', 'Mobile App', 'Dashboard', 'Landing Page', 'E-commerce', 'Portfolio', 'Desktop App', 'Custom'];
export const EXPORT_PRESETS = [{ id: 'original', label: 'Original', w: 0, h: 0 }, { id: 'portfolio', label: 'Portfolio', w: 1200, h: 800 }, { id: 'portfolio-wide', label: 'Portfolio Wide', w: 1600, h: 900 }, { id: 'case-study', label: 'Case Study', w: 1440, h: 1080 }, { id: 'linkedin', label: 'LinkedIn Post', w: 1200, h: 627 }, { id: 'linkedin-cover', label: 'LinkedIn Cover', w: 1584, h: 396 }, { id: 'upwork', label: 'Upwork', w: 1280, h: 960 }, { id: 'square', label: 'IG Square', w: 1080, h: 1080 }, { id: 'portrait', label: 'IG Portrait', w: 1080, h: 1350 }, { id: 'story', label: 'IG Story', w: 1080, h: 1920 }, { id: 'hd', label: 'Full HD', w: 1920, h: 1080 }, { id: '4k', label: '4K', w: 3840, h: 2160 }];
...
export const LIGHTING: { id: LightType; label: string }[] = [{ id: 'none', label: 'None' }, { id: 'top', label: 'Top light' }, { id: 'bottom', label: 'Riser glow' }, { id: 'left', label: 'Left key' }, { id: 'right', label: 'Right key' }, { id: 'center', label: 'Center glow' }, { id: 'ambient', label: 'Ambient' }];
export const TECH_BADGES = ['React', 'TypeScript', 'Node.js', 'Next.js', 'Vue', 'Angular', 'Svelte', 'MongoDB', 'PostgreSQL', 'MySQL', 'Tailwind', 'Express', 'Python', 'Django', 'Firebase', 'Supabase', 'Flutter', 'React Native', 'Figma', 'GraphQL', 'Docker', 'AWS', 'Vite', 'Redux', 'Prisma', 'Rust', 'Go', 'Swift', 'Kotlin', 'Java'];
export interface TypoPreset { id: string; label: string; scale: number; pos: PosPreset; badges: boolean; spacingNote: string }
export const TYPO_PRESETS: TypoPreset[] = [{ id: 'saas', label: 'Modern SaaS', scale: 1.0, pos: 'bottom-left', badges: true, spacingNote: 'balanced' }, { id: 'editorial', label: 'Editorial', scale: 1.35, pos: 'top-left', badges: false, spacingNote: 'wide' }, { id: 'minimal', label: 'Minimal', scale: 0.8, pos: 'bottom-center', badges: false, spacingNote: 'airy' }, { id: 'bold', label: 'Bold', scale: 1.5, pos: 'center-left', badges: true, spacingNote: 'tight' }, { id: 'technical', label: 'Technical', scale: 0.9, pos: 'bottom-left', badges: true, spacingNote: 'mono' }, { id: 'luxury', label: 'Luxury', scale: 1.2, pos: 'bottom-right', badges: false, spacingNote: 'serif' }, { id: 'developer', label: 'Developer', scale: 0.95, pos: 'top-left', badges: true, spacingNote: 'mono' }, { id: 'corporate', label: 'Corporate', scale: 1.05, pos: 'bottom-left', badges: true, spacingNote: 'clean' }];
export const SHADOWS: { id: ShadowPreset; label: string; dx: number; dy: number; blur: number; alpha: number }[] = [{ id: 'none', label: 'None', dx: 0, dy: 0, blur: 0, alpha: 0 }, { id: 'soft', label: 'Soft', dx: 0, dy: 22, blur: 55, alpha: 0.38 }, { id: 'hard', label: 'Hard', dx: 14, dy: 18, blur: 3, alpha: 0.42 }, { id: 'float', label: 'Float', dx: 0, dy: 44, blur: 90, alpha: 0.48 }, { id: 'glow', label: 'Glow', dx: 0, dy: 12, blur: 70, alpha: 0.5 }, { id: 'product', label: 'Product', dx: 0, dy: 30, blur: 42, alpha: 0.42 }, { id: 'cinematic', label: 'Cinematic', dx: 0, dy: 60, blur: 120, alpha: 0.55 }, { id: 'long', label: 'Long', dx: 26, dy: 40, blur: 26, alpha: 0.3 }];
...
export const DECO_PRESETS: DecoPresetDef[] = [
  dp('circle', 'Circle', 'geometric', 'disc'), dp('ring', 'Ring', 'geometric', 'ring'), dp('square', 'Square', 'geometric', 'square'), dp('triangle', 'Triangle', 'geometric', 'triangle'), dp('line', 'Line', 'geometric', 'line'), dp('arc', 'Arc', 'geometric', 'arc'), dp('dot', 'Dot field', 'geometric', 'dotgrid'), dp('plus', 'Plus', 'geometric', 'plus'), dp('orbit', 'Orbit', 'geometric', 'orbit'),
  dp('sphere', 'Sphere', '3d', 'sphere'), dp('cube', 'Cube', '3d', 'cube'), dp('torus', 'Torus', '3d', 'torus'), dp('pill', 'Pill', '3d', 'pill'), dp('blob3d', 'Soft blob', '3d', 'blob'), dp('disc3d', 'Disc', '3d', 'disc'), dp('ribbon', 'Ribbon', '3d', 'ribbon'),
  dp('wave', 'Wave', 'abstract', 'wave'), dp('glow-orb', 'Glow orb', 'abstract', 'sphere'), dp('gradient-orb', 'Gradient orb', 'abstract', 'blob'), dp('sparkle', 'Sparkle', 'abstract', 'sparkle'), dp('soft-shadow', 'Soft shadow', 'abstract', 'disc'),
  dp('glass-card', 'Glass card', 'ui', 'glasscard'), dp('ui-panel', 'UI panel', 'ui', 'uipanel'), dp('notification', 'Notification', 'ui', 'notification'), dp('chart-card', 'Chart card', 'ui', 'chart'),
];

export function getDecoShapes(set: DecoSet, seed: number, w: number, h: number, intensity: number, c1: string, c2: string): DecoShape[] {
  if (set === 'none') return [];
  const rnd = mulberry32(seed); const out: DecoShape[] = []; const min = Math.min(w, h); const colors = [c1, c2, '#f2f0ea'];
  const edgePos = () => { const side = Math.floor(rnd() * 4); const m = 0.16; if (side === 0) return { x: rngRange(rnd, -0.04, m), y: rngRange(rnd, 0.02, 0.98) }; if (side === 1) return { x: rngRange(rnd, 1 - m, 1.04), y: rngRange(rnd, 0.02, 0.98) }; if (side === 2) return { x: rngRange(rnd, 0.02, 0.98), y: rngRange(rnd, -0.04, m * 0.7) }; return { x: rngRange(rnd, 0.02, 0.98), y: rngRange(rnd, 1 - m * 0.7, 1.04) }; };
  const n = Math.round((set === 'sparkles' ? 10 : 6) * intensity);
  for (let i = 0; i < n; i++) { const p = edgePos(); const color = colors[i % colors.length]; const o = rngRange(rnd, 0.25, 0.7) * Math.min(1, intensity); if (set === 'orbs') out.push({ t: 'circle', x: p.x * w, y: p.y * h, r: rngRange(rnd, 0.03, 0.13) * min * intensity, color, o: o * 0.55, rot: 0 }); else if (set === 'rings') out.push({ t: 'ring', x: p.x * w, y: p.y * h, r: rngRange(rnd, 0.04, 0.15) * min * intensity, color, o, rot: 0 }); else if (set === 'grid') out.push({ t: 'dots', x: p.x * w, y: p.y * h, r: rngRange(rnd, 0.05, 0.11) * min, color, o: o * 0.8, rot: rngRange(rnd, -0.3, 0.3) }); else if (set === 'sparkles') out.push({ t: i % 3 === 0 ? 'plus' : 'sparkle', x: p.x * w, y: p.y * h, r: rngRange(rnd, 0.012, 0.03) * min * intensity, color, o, rot: rngRange(rnd, 0, 0.8) }); else if (set === 'waves') out.push({ t: 'line', x: p.x * w, y: p.y * h, r: rngRange(rnd, 0.08, 0.2) * min, color, o: o * 0.7, rot: rngRange(rnd, -0.5, 0.5) }); }
  return out;
}
...
export function randomizeProject(p: Project): Project {
  const seed = (Date.now() ^ Math.floor(Math.random() * 1e9)) >>> 0; const rnd = mulberry32(seed); const pal = pick(rnd, PALETTES);
  const tilts = [-6, -4, -2, 0, 0, 2, 4, 6]; const shadows: ShadowPreset[] = ['soft', 'soft', 'float', 'product', 'cinematic', 'glow']; const decos: DecoSet[] = ['orbs', 'rings', 'grid', 'sparkles', 'waves', 'none']; const textPos: PosPreset[] = ['bottom-left', 'bottom-center', 'top-left', 'top-center']; const logoPos: PosPreset[] = ['top-right', 'top-left', 'bottom-right', 'bottom-left'];
  return { ...p, updatedAt: Date.now(), background: paletteToBg(pal, Math.floor(rnd() * 1e9)), decoration: { ...p.decoration, set: pick(rnd, decos), seed: Math.floor(rnd() * 1e9), intensity: rngRange(rnd, 0.7, 1.2) }, text: { ...p.text, position: pick(rnd, textPos) }, logo: { ...p.logo, position: p.logo.enabled ? pick(rnd, logoPos) : p.logo.position }, devices: p.devices.map((d, i) => ({ ...d, tilt: pick(rnd, tilts) + (i % 2 === 0 ? 0 : pick(rnd, [-2, 2])), shadow: pick(rnd, shadows) })), accents: { a1: pal.a1, a2: pal.a2 } };
}

export function makeDevice(kind: DeviceKind, cw: number, ch: number, assetId: string | null, index: number): DeviceLayer {
  const meta = DEVICE_META[kind]; const w = cw * meta.defFw; const h = w / meta.aspect;
  return { id: uid(), kind, name: `${meta.label} ${index + 1}`, x: (cw - w) / 2, y: (ch - h) / 2, w, tilt: 0, color: meta.colors[0].hex, assetId, fit: 'cover', zoom: 1, panX: 0, panY: 0, shadow: 'soft', url: 'yourapp.com', visible: true, brightness: 1, reflection: 0, radiusMul: 1, opacity: 1, material: 'matte', z: index };
}

export function makeDefaultProject(name: string, type: string, cw: number, ch: number): Project {
  const preset = BG_PRESETS[0]; const text: TextBlock = { enabled: true, title: '', subtitle: '', showBadges: false, badges: ['React', 'TypeScript', 'Tailwind'], position: 'bottom-left', scale: 1, color: '#f2f0ea', autoColor: true };
  return { id: uid(), name, type, createdAt: Date.now(), updatedAt: Date.now(), canvas: { w: cw, h: ch }, assets: [], devices: [makeDevice('laptop', cw, ch, null, 0)], background: { ...preset.bg, seed: Math.floor(Math.random() * 1e9) }, text, logo: { enabled: false, assetId: null, position: 'top-right', size: 0.08, opacity: 0.9 }, decoration: { set: 'orbs', seed: Math.floor(Math.random() * 1e9), intensity: 1, density: 0.5, layers: [] }, accents: { a1: '#ff6b3d', a2: '#45d6c8' }, thumbnail: null, exportCount: 0, decos: [], mood: 'auto' };
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function migrate(p: any): Project {
  const base = makeDefaultProject('x', 'Custom', 1600, 1000);
  return { ...base, ...p, canvas: { ...base.canvas, ...(p.canvas || {}) }, background: { ...base.background, ...(p.background || {}) }, decoration: { ...base.decoration, ...(p.decoration || {}), layers: (p.decoration?.layers) || [] }, devices: (p.devices || []).map((d: Partial<DeviceLayer> & { id: string }) => ({ brightness: 1, reflection: 0, radiusMul: 1, opacity: 1, material: 'matte' as Material, z: 0, ...d })), decos: p.decos || [], mood: p.mood || 'auto', accents: p.accents || { a1: '#ff6b3d', a2: '#45d6c8' }, text: { ...base.text, ...(p.text || {}) }, logo: { ...base.logo, ...(p.logo || {}) } };
}
...
export function applyLayoutPositions(p: Project, layoutId: string): Project {
  const layout = LAYOUTS.find(l => l.id === layoutId); if (!layout) return p;
  const { w: cw, h: ch } = p.canvas; const assets = p.assets;
  const devices: DeviceLayer[] = layout.devices.map((d, i) => { const meta = DEVICE_META[d.kind]; const w = cw * d.fw; const prev = p.devices[i]; return { ...makeDevice(d.kind, cw, ch, assets[i]?.id ?? null, i), x: cw * d.fx, y: ch * d.fy, w, assetId: assets[i]?.id ?? prev?.assetId ?? null, color: prev?.color ?? meta.colors[0].hex }; });
